...
describe("Rally Web Platform extension interop", function () {
  it("fails to sign into website with invalid credentials", async function () {
    ...
    for (const driver of [getChromeDriver, getFirefoxDriver]) {
      this.driver = await driver(headlessMode, installExtension);
  ...
  it("signs into website and tries all UX flows without extension installed", async function () {
    ...
    for (const driver of [getChromeDriver, getFirefoxDriver]) {
      ...
      await findAndAct(this.driver, By.xpath('//button[text()="Cancel"]'), e => e.click());
      await this.driver.wait(until.elementIsVisible(await this.driver.findElement(By.xpath('//button[text()="Leave Mozilla Rally"]'))), WAIT_FOR_PROPERTY);
      ...
      // await this.driver.wait(until.elementIsVisible(await this.driver.findElement(By.xpath('//button[text()="Accent & Enroll"]'))), WAIT_FOR_PROPERTY);
  ...
  it("signs into website and tries all UX flows with extension installed", async function () {
    ...
    // FIXME
    // for (const driver of [getChromeDriver, getFirefoxDriver]) {
    for (const driver of [getFirefoxDriver]) {
      this.driver = await driver(headlessMode, installExtension);
